<template>
  <div>
    ...
    >
      ...
      <!-- 添加设备按钮 -->
      <el-button @click="addcrop" type="primary">dasdasda添加植物dasdsa</el-button>
    <div>
          ...
          <el-col :span="6" v-for="crop in crops" :key="crop.id">
            <el-card  >
              ...
              <template v-slot:header>
                ...
                <h4 v-if="dqvs"  style="color: green;">读取dsadadasda数据库数据中</h4>
...
// 根据用户名获取croppp    ID列表
  async fetchcropsByUsername(username) {
    ...
    renderRadarChart() {
      ...
      // 提取雷达点数据
      const radarData = this.radarStatuses.map(crop => ({
        name: crop.name,
        value: crop.isRedPoint ? 1 : 0,  // 红点为1，绿点为0
        itemStyle: {
          color: crop.isRedPoint ? 'red' : 'green'  // 根据状态设置颜色
        }
      }));
      ...
      const option = {
        title: {
          text: 'Crop Radar Status',
          left: 'center',
          top: 20
        },
        tooltip: {},
        radar: {
          indicator: this.radarStatuses.map(crop => ({
            name: crop.name,
            max: 1  // 设置最大值为1，红点或绿点都在 0-1 范围内
          })),
          shape: 'circle'
        },
        series: [
          {
            ...
            type: 'radar',
            data: [{
              value: radarData.map(crop => crop.value),
              name: 'Crop Status',
              itemStyle: { color: 'transparent' },  // 使用透明填充
              areaStyle: {
                color: 'rgba(0, 0, 0, 0.1)'  // 设置背景区域的透明色
              }
            }]
          }   
        ]
...
async  cgqsd(cropid) {
     ...
     this,this.fetchSensorData(cropid,"湿度") 
},

async  cgqsz(cropid) {
   
   this,this.fetchSensorData(cropid,"水质") 
},
